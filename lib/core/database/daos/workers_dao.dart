...
class WorkersDao extends DatabaseAccessor<AppDatabase>
  ...
  // Get all active workers - REQUIRES ownerId
  ...
  // Watch all active workers - REQUIRES ownerId
  ...
  // Get worker by name - REQUIRES ownerId
  Future<Worker?> getWorkerByName(String name, {required String ownerId}) async {
    if (ownerId.isEmpty) return null;
  ...
  // Get worker by phone - REQUIRES ownerId
  Future<Worker?> getWorkerByPhone(String phone, {required String ownerId}) async {
    if (ownerId.isEmpty) return null;
  ...
  // Get dirty workers - REQUIRES ownerId
  Future<List<Worker>> getDirtyWorkers({required String ownerId}) {
    if (ownerId.isEmpty) return Future.value([]);
  ...
  Future<Map<String, dynamic>> getWorkerStats(String id, {required String ownerId}) async {
  ...
  Future<int> updateCollectionStats(String id, {double? todayCollected, double? monthTotal}) {
    return (update(workersTable)..where((tbl) => tbl.id.equals(id)))
        .write(WorkersTableCompanion(
          todayCollected: todayCollected != null ? Value(todayCollected) : const Value.absent(),
  ...
  Future<int> countWorkers({required String ownerId}) async {
    ...
    final query = selectOnly(workersTable)
      ...
      ..where(workersTable.isDeleted.equals(false));
