...
function getHandlerProperty(objectExpr) {
    const maybeHandler = objectExpr.properties.find((prop) => prop.type === "Property" &&
        prop.key.type === "Identifier" &&
        prop.key.name === "handler");
    if (!maybeHandler)
        return null;
    if (maybeHandler.value.type === "ArrowFunctionExpression" ||
 ...
 */
exports.requireArgsValidator = (0, util_js_1.createRule)({
    ...
    meta: {
        ...
        messages: {
            ...
            "missing-args": "Convex function is missing args validator but has parameter. Add appropriate args validator.",
        },
        schema: [
            {
                type: "object",
                properties: {
                    ignoreUnusedArguments: {
                        type: "boolean",
                        description: "If true, don’t require args validator when function doesn’t use args parameter",
                    },
                },
        ...
        return {
            VariableDeclarator(node) {
                ...
                if (!(node.init?.type === "CallExpression" &&
                    ...
                    util_js_1.CONVEX_REGISTRARS.includes(node.init.callee.name) &&
                    node.init.arguments.length === 1))
                    return;
                // Old function argument syntax?
                if (node.init.arguments[0].type === "ArrowFunctionExpression" ||
                ...
                if (node.init.arguments[0].type === "ObjectExpression") {
                    const objectArg = node.init.arguments[0];
...
//# sourceMappingURL=require-args-validator.js.map