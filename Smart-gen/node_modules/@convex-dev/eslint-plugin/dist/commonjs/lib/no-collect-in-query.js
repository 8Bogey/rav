...
exports.noCollectInQuery = void 0;
...
function findOrderedQueryType(program, checker) {
    try {
        for (const sf of program.getSourceFiles()) {
            ...
            if (!fname.includes("/convex/"))
                continue;
            if (!fname.endsWith("/server/query.ts") &&
 ...
 */
exports.noCollectInQuery = (0, util_js_1.createRule)({
    name: "no-collect-in-query",
    meta: {
        type: "suggestion",
        docs: {
            description: "Disallow calling `.collect()` on Convex queries; prefer `.take()` or `.paginate()` instead.",
        },
        schema: [],
        hasSuggestions: true,
        messages: {
            "no-collect-in-query": "Avoid calling `.collect()` in a Convex query: it can fail for large datasets. Prefer `.take()` or `.paginate()` instead (see the best practices docs). If you are certain that this call to `.collect()` won’t reach the [Convex query limits](https://docs.convex.dev/production/state/limits), you can disable this line with `// eslint-disable-next-line @convex-dev/no-collect-in-query`.",
                ...
                context.report({
                    ...
                    suggest: [
                        ...
                        },
                        {
                            ...
                            fix: (fixer) => fixer.replaceText(node.callee.property, "paginate"),
                        },
                    ],
                });
            },
...
//# sourceMappingURL=no-collect-in-query.js.map